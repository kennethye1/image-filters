 ...
 *
 *
 * Will execute the first "validated" filter and image
 * Note that if given a wrong filter and a validated filter, it will execute
 * the validated filter on an appropriate image and same thing goes for
 * image. 
 * 
 * For filter_index and img_index:
 *  >=0: indicates that a respective filter or image was found
 * -1: either "image" or "filter" not found (or both)
 * -2: no acceptable pair and a '/' was found on the last entry of respective value
 * -3: no acceptable pair and not executable/readable for respective value
 *  
 *  To execute, following should hold: 
 *  filter_index && img_index >= 0
 * 
 */
void image_filter_response(int fd, const ReqData *reqData) {
    int filter_index, img_index;
    char* path_filter = NULL;
    char* path_image = NULL;
    filter_index = img_index = -1;
    //iterate through params which are not null 
    for(int i = 0; i < MAX_QUERY_PARAMS && reqData->params[i].name != NULL; i++){
        if(filter_index < 0){
            if(strcmp("filter", reqData->params[i].name) == 0){
                char* ret = strchr(reqData->params[i].value, '/');
                if(ret){ //a '/' was found
                    filter_index = -2;
                    continue;
                }
                path_filter = malloc(strlen("filters/") + strlen(reqData->params[i].value) + 1);
                if(path_filter == NULL){
                    perror("malloc");
                    exit(1);
                }
                path_filter[0] = '\0';
                strcat(path_filter, "filters/");
                strcat(path_filter, reqData->params[i].value);
                if(access(path_filter, X_OK) == -1){
                    //check if it is executable, if -1 it isn't.
                    filter_index = -3;
                    free(path_filter);
                    continue;
                }
                //Then a filter was found! Get the value of index
                filter_index = i;
                continue;
            } 
        }
        if(img_index < 0){ // check if an image already been given 
            if(strcmp("image", reqData->params[i].name) == 0){
                char* ret = strchr(reqData->params[i].value, '/');
                if(ret){ //a '/' was found
                    img_index = -2;
                    continue;
                } 
                path_image = malloc(strlen("images/") + strlen(reqData->params[i].value) + 1);
                if(path_image == NULL){
                    perror("malloc");
                    exit(1);
                }
                path_image[0] = '\0';
                strcat(path_image, "images/");
                strcat(path_image, reqData->params[i].value);
                //check if readable
                if(access(path_image, R_OK) == -1){
                    free(path_image);
                    img_index = -3;
                    continue;
                }
    ...
    //if no "image" or "filter" found
    if(img_index == -1 || filter_index == -1){
        bad_request_response(fd, "No query parameter \"image\" or \"filter\" found");
    } else if(img_index == -2 || filter_index == -2){ // '/' was found
        bad_request_response(fd, "A '/' was found for image or filter value(s)");
    } else if(img_index == -3){
        bad_request_response(fd, "No readable image file");
    }else if(filter_index == -3){
        bad_request_response(fd, "No executable filter");
    }else{
        char copy_filt[strlen(path_filter) + 2];
        copy_filt[0] = '\0';
        strcat(copy_filt, "./");
        strcat(copy_filt, path_filter);
        free(path_filter);
        write_image_response_header(fd);
        FILE* file = fopen(path_image, "rb");
        free(path_image);
        ...
        fclose(file);
        execl(copy_filt, reqData->params[filter_index].value, NULL);
    }
}
